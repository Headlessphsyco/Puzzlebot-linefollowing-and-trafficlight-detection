...
class Node(object):
    def __init__(self):
        ...
        self.object_detect = rospy.get_param('/object_detected',False)


        KPtheta = rospy.get_param('/kptheta',0.4)
        KItheta = rospy.get_param('/kitheta',0)
        KDtheta = rospy.get_param('/kdtheta',0)
    ...
    def callback(self, msg):
        ...
        if self.object_detect:
            ...
            # line following code
            etheta = (240 - self.lines.line_point_x[0])/(240*5)
            max_speed = 0.1
	    if abs(etheta)>0.3:
		self.velocities.linear.x = 0
	    else:
		self.velocities.linear.x = max_speed*(1-abs(etheta*5))
