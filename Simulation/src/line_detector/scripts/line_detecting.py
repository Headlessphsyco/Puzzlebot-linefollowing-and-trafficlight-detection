...
class Nodo(object):
    ...
    def callback(self, msg):
        ...
        # cropping image
        self.image = self.image[int(row*0.45):int(row*1), int(col*0.2):int(col*0.8)]
        
        
        # grey scaling and blurring
        gray = cv2.cvtColor(self.image,cv2.COLOR_BGR2GRAY)
        kernel_size = rospy.get_param('/kernel_size', 3)
        blur_gray = cv2.GaussianBlur(gray,(kernel_size, kernel_size),0)
        thresh = rospy.get_param('/binary_threshold', 200)
        ret, thresh2 = cv2.threshold(blur_gray, thresh, 255, cv2.THRESH_BINARY_INV) 
        ...
        edges = cv2.Canny(thresh2, low_threshold, high_threshold)
        

        # setting up parameters for Hough on edge detection
        rho = 1  # distance resolution in pixels of the Hough grid
        theta = np.pi / 180  # angular resolution in radians of the Hough grid
        threshold = rospy.get_param('/line_threshold', 20)  # minimum number of votes (intersections in Hough grid cell)
        ...
        if lines is not None:
            ...
            high_range = rospy.get_param('/midpoint_high', 100)
            low_range = rospy.get_param('/midpoint_low', 10)
            midpoints = []
            # for y in x_values:
        ...
        show_image = rospy.get_param('/show_images',True)
        if show_image:
            cv2.imshow('image', image)
            cv2.imshow('after image processing', thresh2) 
            cv2.imshow('edge detection',edges)
            cv2.imshow('image with line', lines_edges)
            cv2.imshow('lines', line_image)
        cv2.waitKey(33) #delay
